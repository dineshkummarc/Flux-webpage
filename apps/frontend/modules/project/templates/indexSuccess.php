...
<?php else: ?>
	...
	<?php foreach ($projects as $project): ?>
		<?php if ($i > 0) echo '<hr/>'?>
		<?php if (strlen($project->getLogo()) > 0) echo link_to(image_tag($project->getLogo(), array('class' => 'logo', 'alt' => 'Logo '.$project->getName())), '@projectes_show_'.$sf_user->getCulture().'?id='.$project->getId()); ?>
